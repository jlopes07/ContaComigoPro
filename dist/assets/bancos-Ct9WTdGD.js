import{s as n,f as u,c as k,a as E}from"./index-Bf9tqZDu.js";function F(){w(),E(()=>{var a;(a=document.getElementById("page-bancos"))!=null&&a.classList.contains("active")&&w()})}function w(){const a=document.getElementById("banks-list");if(a){if(n.currentBankFilter.id){$(n.currentBankFilter.id);return}if(a.innerHTML="",n.banksList.length===0){a.innerHTML='<div class="empty-state w-100" style="grid-column: 1/-1;"><i class="fa-solid fa-building-columns"></i><p>Nenhuma conta bancária cadastrada.</p></div>';return}n.banksList.forEach(t=>{const s=n.transactions.filter(i=>i.paymentMethod===t.id),o=s.filter(i=>i.type==="income").reduce((i,d)=>i+d.amount,0),r=s.filter(i=>i.type==="expense").reduce((i,d)=>i+d.amount,0),f=(t.balance||0)+o-r,v=f<0?"var(--danger)":"var(--text-main)";a.innerHTML+=`
